...
If the outlier is a highly massive number, it pulls the mean artificially high. If a brilliant student sleeps through an exam and scores a 0, that extremely low outlier pulls the class mean down. When outliers crash the party, the mean stops being an accurate reflection of the "typical" experience. We need a backup plan.
...
### The Superpower of the Median
Remember Elon Musk walking into the room of teachers? If we calculate the *median* salary of that room, we line everyone up from poorest to richest. The middle person is still a teacher making \$50,000. Elon Musk is just the guy at the far right of the line.

Because it relies purely on position, **the median is highly resistant to extreme values, or outliers, in a data set.** When your data is skewed by bizarre anomalies, the median is the most honest storyteller in the room.

---

## 3. The Mode: The Popular Vote
...
### The Quirks of the Mode
Because the mode relies entirely on frequency, it has some unique properties you must watch out for on the Praxis exam:

*   **The "No Mode" Scenario:** What if everyone gets a completely different score on the test? Or what if three students score a 70, three score an 80, and three score a 90? **A data set has no mode if every value in the data set appears with the exact same frequency.**
*   **The "Bimodal" Scenario:** Sometimes, a class is sharply divided. You might have a group of students who studied perfectly, and a group who didn't study at all. **A data set is considered bimodal if exactly two distinct values share the highest frequency of occurrence** (e.g., five students score a 100, and five students score a 60).
*   **The "Multimodal" Scenario:** It doesn't stop at two! **A data set is considered multimodal if more than two distinct values share the highest frequency of occurrence.** 

### The Unique Domain of the Mode
You might be wondering, "Why do we even need the mode if we have the mean and median?" 
...
## 4. Manipulating the Data: The "Shift and Stretch" Rules
...
The exact reverse is true for subtraction:
*   **Subtracting a constant value from every number in a data set decreases the arithmetic mean by that exact constant value.**
...
## 5. The Shape of the Data: Skewness and the Tug-of-War

We’ve calculated the numbers, but let's visualize them. The relationship between the mean, median, and mode tells us an incredible story about the *shape* of our data distribution. 

### Symmetrical Data: Perfect Harmony
Imagine a perfectly normal, bell-shaped curve. Most students scored a "C", a few scored "B"s and "D"s, and an equal, tiny number scored "A"s and "F"s. 

> **In a perfectly symmetrical, unimodal data distribution, the arithmetic mean, median, and mode share the exact same numerical value.**

They are all sitting happily together right at the very peak of the bell curve. There are no outliers pulling the mean, and the weights on both sides are perfectly identical.

### Skewed Data: The Tug-of-War
But the real world is rarely perfectly symmetrical. Often, exams are slightly too hard or slightly too easy, creating a "tail" of outliers that pulls the data in one direction.
...
Now imagine a ridiculously easy elementary school spelling test. Almost everyone gets a perfect 100 (the mode is high). But a couple of kids slept entirely through the exam and scored a 0. 

These extremely low outliers create a long "tail" pointing to the left side of the graph. Because these low scores pull the arithmetic mean down toward zero, the mean drops below the sturdy median.
*   **In a left-skewed data distribution, the arithmetic mean is typically less than the median.**
...
## Summary for the Praxis Core

To conquer Measures of Central Tendency on the Praxis Core Math exam, remember the distinct personality of each measure:

1.  **The Mean** ($\mu$ or $\bar{x}$) is your mathematical balancer. It is precise, but it is deeply vulnerable to the gravitational pull of **outliers**.
2.  **The Median** is the stubborn center of an ordered list. It ignores the noise of extreme high or low values, making it highly robust. 
3.  **The Mode** is simply a count of frequency. It might be bimodal, multimodal, or not exist at all—but it is your only hope when dealing with categorical, non-numerical data.
4.  **Transformations** (adding, subtracting, multiplying, dividing by constants) alter the mean, median, and mode identically. The center always goes exactly where the rest of the data goes.
5.  **The Shape** defines the relationship. Perfect symmetry means they are equal. A skewed tail pulls the arithmetic mean relentlessly toward it, dragging it above or below the median.

Data isn't just a lifeless pile of numbers. It’s a story about your classroom, your students, and the world. Once you know how to locate its center, you finally know how to read the story! Happy studying!