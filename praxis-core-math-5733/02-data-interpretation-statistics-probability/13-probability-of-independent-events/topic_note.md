...
Today, we are going to talk about predicting the future. We aren’t going to use a [crystal ball](https://en.wikipedia.org/wiki/Crystal_ball), of course. We are going to use something far more powerful, far more elegant, and beautifully logical. We are going to use [mathematics](https://en.wikipedia.org/wiki/Mathematics). Specifically, we are diving into the world of **[probability](https://en.wikipedia.org/wiki/Probability)**, a concept that governs everything from the [weather forecast](https://en.wikipedia.org/wiki/Weather_forecasting) to the [genetic traits](https://en.wikipedia.org/wiki/Phenotypic_trait) of a blooming flower. 
...
Before we start [flipping coins](https://en.wikipedia.org/wiki/Coin_flipping) or dealing [cards](https://en.wikipedia.org/wiki/Playing_card), we have to establish the rules of our universe. 

Fundamentally, **[probability](https://en.wikipedia.org/wiki/Probability) is a numerical measure representing the [likelihood](https://en.wikipedia.org/wiki/Likelihood_function) of a specific [event](https://en.wikipedia.org/wiki/Event_%28probability_theory%29) occurring.** It is our mathematical way of domesticating the unknown. 

Now, nature doesn’t deal in infinites; it works within strict boundaries. **The probability of an event is always expressed as a [real number](https://en.wikipedia.org/wiki/Real_number) between zero and one inclusive.** You cannot have a [negative probability](https://en.wikipedia.org/wiki/Negative_probability), and you cannot give "110% effort" in the strict mathematical sense!
...
When we talk about consecutive events, we have to ask whether one event cares about the other. By definition, **two events are [independent](https://en.wikipedia.org/wiki/Independence_%28probability_theory%29) if the occurrence of the first event does not change the probability of the second event occurring.** 

Let's look at the classic examples:

### The Coin Toss
**Tossing a standard [coin](https://en.wikipedia.org/wiki/Coin) multiple times produces independent events.** The coin has no [brain](https://en.wikipedia.org/wiki/Brain). It doesn't remember what it did ten seconds ago. **The outcome of a [coin toss](https://en.wikipedia.org/wiki/Coin_flipping) is unaffected by previous tosses of that same coin.** Therefore, **the [theoretical probability](https://en.wikipedia.org/wiki/Probability) of getting heads on a single flip of a [fair coin](https://en.wikipedia.org/wiki/Fair_coin) is exactly one-half** ($0.5$ or $\frac{1}{2}$), whether it's the first flip or the millionth flip.

![A standard coin toss is a classic example of an independent event; the coin retains no memory of previous flips, meaning the probability of landing on heads remains exactly 0.5 every time.](https://wikipedia.org/wiki/Special:Redirect/file/Coin_Toss_(3635981474).jpg)
...
![A standard six-sided die provides a 1 in 6 chance for any specific number to land face up. Like a coin, each roll is completely independent of the last.](https://wikipedia.org/wiki/Special:Redirect/file/6sided_dice_(cropped).jpg)

---

## 3. The Magic Word: "AND"

Often, we don't just care about one isolated event. We want to know the [odds](https://en.wikipedia.org/wiki/Odds) of a sequence of things happening. I want to flip heads *and* then roll a 6. 
...
Whenever you are reading a [Praxis](https://en.wikipedia.org/wiki/Praxis_test) math problem and you see the word "and," I want alarm bells to ring in your head. **The word 'and' in [probability word problems](https://en.wikipedia.org/wiki/Word_problem_%28mathematics_education%29) generally indicates the [intersection](https://en.wikipedia.org/wiki/Intersection_%28set_theory%29) of multiple events.** 

### How Do We Calculate Joint Probability?

Let's use our intuition. If I have half ($\frac{1}{2}$) of a [pie](https://en.wikipedia.org/wiki/Pie), and I want to take a third ($\frac{1}{3}$) of *that* half... what do I do mathematically? I multiply! Half of a third is a sixth ($\frac{1}{6}$). 
...
> **The [Multiplication Rule](https://en.wikipedia.org/wiki/Independence_%28probability_theory%29) for Independent Events:**
...
You might also see this written with [set notation](https://en.wikipedia.org/wiki/Set-builder_notation). **The [mathematical notation](https://en.wikipedia.org/wiki/Notation) $P(A \cap B)$ represents the probability of both event A and event B occurring.** That little arch symbol ($\cap$) means "intersection"—which is just the mathematician's shorthand for "and"!

![A Venn diagram visualizing the intersection of two sets or events. The central overlapping region represents the joint probability of both Event A and Event B occurring simultaneously, mathematically denoted as P(A ∩ B).](https://wikipedia.org/wiki/Special:Redirect/file/Venn_A_intersect_B.svg)
...
Now, let's test your intuition with a [deck of cards](https://en.wikipedia.org/wiki/Playing_card). This is a favorite trick of exam writers. 
...
*   **A standard playing card deck contains exactly four [suits](https://en.wikipedia.org/wiki/Suit_%28cards%29)** ([Hearts](https://en.wikipedia.org/wiki/Playing_card_suit), [Diamonds](https://en.wikipedia.org/wiki/Playing_card_suit), [Clubs](https://en.wikipedia.org/wiki/Playing_card_suit), [Spades](https://en.wikipedia.org/wiki/Playing_card_suit)).

![A standard 52-card deck is divided into four distinct suits. Drawing cards from this specific sample space will change the probabilities for subsequent draws unless the drawn card is returned to the deck.](https://wikipedia.org/wiki/Special:Redirect/file/Svg-cards-2.0.svg)
...
Whenever you see a problem involving drawing [marbles](https://en.wikipedia.org/wiki/Marble_%28toy%29) from a bag, or cards from a deck, immediately scan for the words "with replacement" or "without replacement." It changes the entire mathematical landscape.
...
![A probability tree diagram visually maps out sequential events as branching paths. To calculate the joint probability of reaching a specific final outcome, you simply multiply the individual probabilities along that path.](https://wikipedia.org/wiki/Special:Redirect/file/Probability_tree_diagram.svg)
...
**Two events are [mutually exclusive](https://en.wikipedia.org/wiki/Mutually_exclusive_events) if the two events cannot occur at the same time.** 
