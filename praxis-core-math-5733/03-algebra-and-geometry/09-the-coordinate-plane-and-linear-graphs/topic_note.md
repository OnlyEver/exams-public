...
## The Arena: The Cartesian Coordinate System
...
1.  **The first number in an ordered pair represents the horizontal distance of a point from the origin along the x-axis.**
2.  **The second number in an ordered pair represents the vertical distance of a point from the origin along the y-axis.**
...
Our number lines extend infinitely in both directions, which means we need a way to tell forward from backward. We use positive and negative [signs](https://en.wikipedia.org/wiki/Sign_%28mathematics%29) to indicate direction:

*   **Moving to the right of the y-axis corresponds to a positive x-coordinate.**
*   **Moving to the left of the y-axis corresponds to a negative x-coordinate.**
*   **Moving above the x-axis corresponds to a positive y-coordinate.**
*   **Moving below the x-axis corresponds to a negative y-coordinate.**
...
## Enter the Line: The Magic of Linear Equations

Now that we have our grid, let's look at the [objects](https://en.wikipedia.org/wiki/Mathematical_object) living inside it. The simplest and most elegant object in algebra is the line. 
...
> **Slope is calculated as the [ratio](https://en.wikipedia.org/wiki/Ratio) of the vertical change (rise) to the horizontal change (run) between any two points on a given line.**
...
You can look at a line and instantly know the "flavor" of its slope just by seeing how it leans, reading it like a book from left to right.

*   **Positive Slope:** If you are hiking uphill from left to right, you have a positive slope. **A line with a positive slope slants upward from left to right across the coordinate plane.** 
*   **Negative Slope:** If you are sliding downhill from left to right, the math goes negative. **A line with a negative slope slants downward from left to right across the coordinate plane.**

### The Extremes: Flatlands and Cliffs

What happens when we push the slope to its absolute limits?

**Scenario A: The Flatland.** 
...
Just like the horizontal line ignored $x$, the vertical line ignores $y$. It is locked onto a single $x$ location forever. **The equation of a vertical line is written in the form $x = c$, where $c$ is a constant number.** (For example, $x = -3$).

***

## The Three Languages of Lines: Forms of Linear Equations

A linear equation can put on different outfits depending on what information it wants to show off. On the Praxis, you need to recognize all three of these standard forms.
...
*   **In the slope-intercept equation $y = mx + b$, the variable $b$ represents the y-intercept of the line.** (The constant dangling at the end tells you exactly where the line hits the y-axis).

**How to graph it:**
**To plot a line from slope-intercept form, start by graphing the y-intercept point on the y-axis and then apply the slope ratio to find a second point.** 
...
### 2. Standard Form (The Neat Freak)
Sometimes, mathematicians like to tidy up and get both variables on the same side of the equal sign. 

> **The standard form of a linear equation is represented by the formula $Ax + By = C$.**

In this form, $A$, $B$, and $C$ are usually [integers](https://en.wikipedia.org/wiki/Integer) (no [fractions](https://en.wikipedia.org/wiki/Fraction) allowed). While it doesn't shout the slope at you like $y = mx + b$ does, Standard Form is absolutely brilliant for quickly finding both the x-intercept and the y-intercept. Just cover up the $x$ term to solve for the y-intercept, and cover up the $y$ term to solve for the x-intercept!

### 3. Point-Slope Form (The Utility Player)
What if you are given a random point on a line and the slope, but you *don't* know the y-intercept? You use the point-slope form. 

> **The point-slope form of a linear equation is represented by the formula $y - y_1 = m(x - x_1)$.**

Here, $m$ is your slope, and $(x_1, y_1)$ is any specific, known point on your line. It might look clunky, but it is incredibly powerful for building an equation from scratch. If the Praxis exam says, "Find the equation of a line with a slope of 4 that passes through (2, 5)," you drop those numbers straight into this formula: $y - 5 = 4(x - 2)$. 
...
Master these rules, and you won't just memorize algebra—you will *see* it.