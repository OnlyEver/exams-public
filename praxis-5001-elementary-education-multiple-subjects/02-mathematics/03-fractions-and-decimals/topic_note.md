...
At its core, **a [fraction](https://en.wikipedia.org/wiki/Fraction) represents a part of a whole or a [proportion](https://en.wikipedia.org/wiki/Proportionality_%28mathematics%29) of a set.** It is a way of expressing a relationship between the pieces we care about and the total pieces that exist. 
...
If you want to understand complex matter, you look at its [atoms](https://en.wikipedia.org/wiki/Atom). If you want to understand fractions, you look at unit fractions. 

**A unit fraction is a fraction with a numerator of exactly [one](https://en.wikipedia.org/wiki/1).** Examples include 1/2, 1/4, and 1/17. They are the fundamental building blocks of all other fractions. Why? Because **every fraction can be represented as the sum of multiple unit fractions possessing the same denominator.**
...
[Numbers](https://en.wikipedia.org/wiki/Number) on a page are [abstract](https://en.wikipedia.org/wiki/Abstraction_%28mathematics%29). To build genuine comprehension, we must map these numbers to physical space and geometry. 

### Concrete and Area Models
Before a student ever manipulates symbols on a [chalkboard](https://en.wikipedia.org/wiki/Blackboard), they need to feel the math. **Concrete fraction models use physical manipulatives like fraction strips to physically represent parts of a whole.** When a student holds a "1/2" strip and places two "1/4" strips right underneath it to see they are exactly the same [length](https://en.wikipedia.org/wiki/Length), magic happens.

We then graduate to drawing. **An area model represents a fraction by dividing a two-dimensional geometric shape into equal-sized regions.** 
...
Here is the beautiful mechanical link: **An improper fraction can be rewritten as a mathematically equivalent mixed number.** If you have 5/4 (five one-fourth slices), you have enough to build one complete pie (4/4), with 1/4 left over. Hence, $5/4 = 1 \frac{1}{4}$.
...
Decimals normalize fractions to powers of 10. But human beings love the number 100. It's a nice, round, substantial number. Enter the percent.
...
*   **A value of 25 percent is mathematically equivalent to the fraction 1/4.** (Think of a quarter in relation to a dollar).
