...
Here is a delightful mathematical coincidence: **a whole-number exponent on the base 10 corresponds exactly to the number of zeros that follow the digit 1 in the standard form of the number.** 

Let's look at the foundational powers of 10 you must know:
...
> *   **Division:** Dividing a decimal number by a power of 10 moves the decimal point to the **left** by the number of zeros in the power of 10. (e.g., $45.6 \div 10^1$. $10^1$ has one zero. Move the decimal one spot left: **4.56**)

*Why does this matter?* Because it connects perfectly back to our Golden Rule! If you move a decimal to the right, you are effectively pushing all the digits to the left—making them 10 times larger for every jump. 
...
1.  **Find the Target:** Identify the place value you are rounding to. 
2.  **Look to the Right:** **The digit immediately to the right of the target rounding place dictates whether the target digit rounds up or stays the same.** 

Here are the hard-and-fast rules:
*   **A target rounding digit remains unchanged when the digit immediately to its right is 0, 1, 2, 3, or 4.** (Think: "Four or less, let it rest.")
*   **A target rounding digit increases by exactly one when the digit immediately to its right is 5, 6, 7, 8, or 9.** (Think: "Five or more, raise the score.")

### The Cleanup: What Happens to the Rest of the Number?

Once you've adjusted (or left alone) your target digit, you have to clean up the digits to the right. *However, the cleanup rules change depending on whether you are working with whole numbers or decimals.*
